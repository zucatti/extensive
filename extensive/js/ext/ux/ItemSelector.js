...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    msHeight:300,
    hideNavIcons:false,
    iconUp:"ux-button-arrow-up",
    iconDown:"ux-button-arrow-down",
    iconLeft:"ux-button-arrow-left",
    iconRight:"ux-button-arrow-right",
    iconTop:"ux-button-arrow-top",
    iconBottom:"ux-button-arrow-bottom",
    drawUpIcon:false,
    drawDownIcon:false,
    drawLeftIcon:true,
    drawRightIcon:true,
    ...
    onRender: function(ct, position){
        ...
        var el=icons.getEl();
        if (!this.toSortField) {
            this.toTopIcon = el.createChild({tag:'img', src: Ext.BLANK_IMAGE_URL, 'class':this.iconTop, style:{cursor:'pointer', margin:'2px'}});
            el.createChild({tag: 'br'});
            this.upIcon = el.createChild({tag:'img', src: Ext.BLANK_IMAGE_URL, 'class':this.iconUp, style:{cursor:'pointer', margin:'2px'}});
            el.createChild({tag: 'br'});
        }
        this.addIcon = el.createChild({tag:'img', src: Ext.BLANK_IMAGE_URL, 'class':this.switchToFrom?this.iconLeft:this.iconRight, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        this.removeIcon = el.createChild({tag:'img', src: Ext.BLANK_IMAGE_URL, 'class':this.switchToFrom?this.iconRight:this.iconLeft, style:{cursor:'pointer', margin:'2px'}});
        el.createChild({tag: 'br'});
        if (!this.toSortField) {
            this.downIcon = el.createChild({tag:'img', src: Ext.BLANK_IMAGE_URL, 'class':this.iconDown, style:{cursor:'pointer', margin:'2px'}});
            el.createChild({tag: 'br'});
            this.toBottomIcon = el.createChild({tag:'img', src: Ext.BLANK_IMAGE_URL, 'class':this.iconBottom, style:{cursor:'pointer', margin:'2px'}});
        }
        if (!this.readOnly) {
            if (!this.toSortField) {
